...
public class CityContoller {
    ...
    {
        List<ItripAreaDic> list=itripAreaDicMapper.gethot(t);
        if (list!=null) {
...
//查询商圈
    ...
    @ResponseBody
    public  Dto GetArea(@PathVariable("cityId") Integer cityId)
    {
        List<ItripAreaDic> list=itripAreaDicMapper.getByparent(cityId);
        if (list!=null) {
            return DtoUtil.returnDataSuccess(list);
        }
        return  DtoUtil.returnFail("查询失败!","10202");
